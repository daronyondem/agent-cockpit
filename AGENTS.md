...
- Welcome setup-auth changes for Claude/Codex must preserve system-wide CLI auth reuse: setup profiles should use the user's normal vendor CLI auth home so terminal `claude`/`codex` and Agent Cockpit share login state. Keep explicit account-profile isolation available for users who configure separate profiles.
