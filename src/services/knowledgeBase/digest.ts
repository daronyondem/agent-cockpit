...
export function buildDigestPrompt(input: BuildDigestPromptInput): string {
  ...
  const header = [
    ...
    `- If the converted text contains image references (e.g. \`![Page 1](pages/page-0001.png)\`),`,
    `  you MUST open and analyze EVERY referenced image using your Read tool. Image paths`,
    `  are relative to the converted text file's directory. Your understanding must come`,
    `  from BOTH the markdown and the images — not just one. For rasterized PDFs the`,
    `  markdown is only an index of page images, so the page images ARE the content;`,
    `  failing to read them means you have nothing to digest.`,
    ``,
    `## Source file`,
    ``,
