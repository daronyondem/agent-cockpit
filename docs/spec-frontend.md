...
- **Progress group collapsing:** `chatRenderMessages()` walks the session messages and collapses runs of consecutive assistant messages with `turn === 'progress'` (same `backend`, not interrupted by a user/system/memory_update message) into a **single compact breadcrumb pill** via `chatRenderProgressGroup()` — a `<details>` element with no avatar, no role header, and no action buttons. Collapsed by default: the summary shows a caret, the step count (e.g. "4 steps"), the elapsed time computed against the preceding user message using the *last* progress row's timestamp (so it keeps climbing as more rows land), and a truncated single-line preview of the last row's text. Expanded: reveals a vertical timeline (`.chat-progress-timeline`) where each row holds a per-row timestamp, the row's `chatRenderThinkingBlock`/`chatRenderToolActivityBlock` output, and the row's markdown body. Assistant messages with `turn === 'final'` (or legacy messages without `turn` — treated as final for back-compat) always render as their own full card via `chatRenderSingleMessage()`. Rationale: the final message is what matters; the intermediate chatter should take minimal vertical space but remain accessible on demand.
