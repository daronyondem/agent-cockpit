...
- **Progress group collapsing:** `chatRenderMessages()` walks the session messages and collapses runs of consecutive assistant messages with `turn === 'progress'` (same `backend`, not interrupted by a user/system/memory_update message) into a single grouped card via `chatRenderProgressGroup()`. The group card shows one avatar, one role header (with the group's first timestamp and an elapsed label computed against the preceding user message using the *last* progress row's timestamp — so the elapsed keeps climbing as more progress rows land), and a vertical timeline (`.chat-progress-timeline`) where each row holds a compact per-row timestamp, the row's `chatRenderThinkingBlock`/`chatRenderToolActivityBlock` output, and the row's markdown body. Assistant messages with `turn === 'final'` (or legacy messages without `turn` — treated as final for back-compat) always render as their own full card via `chatRenderSingleMessage()`. Copy / Copy MD on the group card operate on the combined text from all rows (the group's `data-raw-content` concatenates every row's `content` with blank-line separators).
