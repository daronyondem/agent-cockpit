...
- **Synthesis tab:** Interactive 3D WebGL force-directed graph (via [3d-force-graph](https://github.com/vasturiano/3d-force-graph) + Three.js) with an UnrealBloomPass glow effect, showing topics as nodes and topic-to-topic connections as edges. Dependencies are **lazy-loaded** only when the Synthesis tab opens — zero impact on initial page load. Uses the same split architecture as the official 3d-force-graph bloom example: UMD script tag for the graph (`public/js/vendor/3d-force-graph.min.js`, ~1.3MB, bundles its own Three.js) and a native ESM import for the bloom pass (UnrealBloomPass from vendored Three.js addons in `public/js/vendor/three-addons/`). A `<script type="importmap">` in `index.html` maps bare `"three"` specifier to the vendored `three.module.min.js` (~351KB) so that the addon source files can resolve their Three.js imports. Loaded in `main.js` via `_loadForceGraph3D()` (UMD script injection) and `_loadBloom()` (dynamic `import()`). Action bar above the graph has **Dream** (incremental) and **Re-Dream (full rebuild)** buttons, a status line (last run time, pending count, last error), the dream pipeline stepper (when running), and a search input. Dream/Re-Dream buttons return a 400 error (shown via `chatShowAlert`) when there are no entries to process. Dream/Re-Dream buttons are disabled while a dream is running. The Re-Dream button shows a destructive confirmation dialog before proceeding. The tab auto-fetches synthesis data from `GET /workspaces/:hash/kb/synthesis` on activation. **Dream/Re-Dream grace period:** when the Dream or Re-Dream button is clicked, `chatKbBrowserState._dreamTriggeredAt` is set to the current timestamp. For 15 seconds after that timestamp, the poll timer treats the dream status as "running" without overriding the optimistic UI state — this prevents the 1500ms poll from reverting the button/stepper to an idle state before the server has registered the newly-started dream job.
  - **Graph nodes:** 3D spheres whose size scales by entry count (nodeVal = `4 + 6 * (entryCount / maxEntries)`). God-node topics render in gold (`#ffcc00`); regular topics in blue (`#4499ff`). Bloom post-processing adds a neon glow effect to all nodes. Topic title + entry/connection counts are shown via hover tooltip (`nodeLabel`).
  - **Graph edges:** 3D links styled by confidence level — `extracted` links are wider (1.2px) with directional animated particles, `inferred`/`speculative` links are thinner (0.5px). All edges use a muted blue color (`#6688cc`) with varying opacity (0.6/0.4/0.2 by confidence). Directional arrow indicators on all links. No edge labels (relationship text is shown in the node click popup).
  - **Bloom effect:** UnrealBloomPass with strength=3, radius=1, threshold=0 (bloom everything for neon glow). Dark background (`#000008`) for optimal bloom visibility. The bloom pass is added to the graph's built-in `postProcessingComposer()` EffectComposer pipeline. Camera centering uses a manual centroid + bounding-sphere computation (`centerGraph()`) called at 300ms (instant), 2s (animated), and on engine stop — `zoomToFit()` is not used due to unreliability.
  ...
  - **Search:** Text input filters nodes by title — matching nodes keep their color, non-matching nodes dim to near-invisible (`rgba(30, 30, 50, 0.15)`). Unrelated edges fade to 0.03 opacity. The camera flies to the first matching node via `graph.cameraPosition()` with a 1000ms transition.
