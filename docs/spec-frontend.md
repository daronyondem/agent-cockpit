...
- **Layout:** A `<div class="chat-kb-browser">` sibling of `#chat-messages` with a header (workspace label + KB counter pills + close button), a tab row (**Raw** / **Entries** / **Synthesis** — all three wired), and a tab-specific body. Opening the browser hides `#chat-messages` and the input area; closing restores them.
...
- **Synthesis tab:** Grid of topic cards, each showing title, summary snippet (2-line clamp), entry count, and connection count. God-node topics display a star badge. Action bar above the grid has **Dream** (incremental) and **Re-Dream (full rebuild)** buttons plus a status line (last run time, pending count, last error). Dream/Re-Dream buttons return a 400 error (shown via `chatShowAlert`) when there are no entries to process. Clicking a topic card navigates to a detail view with full prose content (rendered as markdown), an entry list, and a connections list (showing related topic name, relationship label, and confidence badge). A **Back to topics** button returns to the grid. Dream/Re-Dream buttons are disabled while a dream is running. The Re-Dream button shows a destructive confirmation dialog before proceeding. The tab auto-fetches synthesis data from `GET /workspaces/:hash/kb/synthesis` on activation. **Dream pipeline stepper:** when a dream is running, a two-step pipeline stepper appears next to the buttons showing `Discovery N/M` → `Synthesis N/M` with the active phase highlighted (purple background), completed phases showing a checkmark (green), and upcoming phases dimmed. An elapsed timer (e.g., "— 2:34") appears next to the active step and resets each time the phase or done count changes. The backend emits an initial `done: 0` frame before the first CLI call, so "Starting…" only appears for 1-2 seconds. Progress data comes from `kb_state_update` `dreamProgress` frames.
- **Dream banner** (chat input area): A `#chat-dream-banner` div above the model-select row in the chat input wrapper. Behavior:
  ...
  - When `conv.kb.dreamingStatus === 'running'`: shows "Dreaming in progress" + the same pipeline stepper used by the Synthesis tab (Discovery → Synthesis with active/done/pending states and batch counters).
  - Otherwise hidden.
