...
**Per-image AI conversion (`src/services/knowledgeBase/ingestion/pageConversion.ts`):** Scaffolding for the hybrid AI-assisted ingestion design (see `docs/design-kb-ingestion-hybrid.md`). Exports `convertImageToMarkdown(imagePath, { adapter, model?, effort?, timeoutMs? })` — a single retry-once wrapper over `adapter.runOneShot` that converts one image (PDF page render, slide render, embedded DOCX figure, or standalone uploaded image) into clean Markdown using the **unified image-to-Markdown prompt** (design §8). Behavior:
...
- Retries once if `runOneShot` throws **or** returns empty/whitespace output. Throws the last error after a second failure — handlers decide whether to fall back to `source: image-only`.
