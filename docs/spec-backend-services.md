...
- **Conversion output** — Each handler writes into `converted/<rawId>/`:
  - `text.md` — extracted text (or image index) with an `# <filename>` header. PDF is a per-page hybrid (`## Page N` sections with `> source: pdfjs|artificial-intelligence|image-only` annotations and a `![Page N](pages/page-NNNN.png)` link on every page) — see the `pdf.ts` handler entry below for the full classification flow. DOCX is GFM-flavored markdown from pandoc (tables preserved); each embedded image reference is followed by a quoted `> Image description (source: artificial-intelligence): …` block when the Ingestion CLI was able to describe it (skipped for icons under 100 px wide and when no Ingestion CLI is configured). PPTX has `## Slide N` sections with optional `### Speaker Notes` plus a `## Embedded Media` block. Passthrough preserves markdown as-is or wraps other text in a code fence tagged with the file extension; full content is preserved regardless of file size.
