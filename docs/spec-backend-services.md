...
- **`synthesis_reflections`** — `{ reflection_id TEXT PRIMARY KEY, title TEXT NOT NULL, type TEXT NOT NULL, summary TEXT, content TEXT NOT NULL, created_at TEXT NOT NULL }`. One row per reflection. `type` is one of `pattern | contradiction | gap | trend | insight`.
...
- **Reflection (Phase E):** A 5th phase that runs after discovery as the final stage before god-node detection. `_runReflection()` generates cross-cluster insights by examining groups of connected topics.
  - **Deterministic checks** — Before LLM calls, two cleanup passes run: (1) **Stale topic detection** — identifies topics whose prose (`content`) is older than the newest assigned entry's `digested_at`, marking them for re-synthesis in a future run. (2) **Delete stale reflections** — any existing reflection whose cited entries have been changed or deleted since the reflection was created is removed via `db.deleteReflections(staleIds)` (using `db.listStaleReflectionIds()`).
  ...
  - **Storage** — Existing reflections are wiped before regenerating (`db.wipeReflections()`). New reflections are stored in the `synthesis_reflections` table with their citations in `synthesis_reflection_citations`. Markdown files are written at `synthesis/reflections/<id>.md` via `regenerateReflectionMarkdown()`.
  ...
  - **KbDatabase methods** — `insertReflection()` writes a reflection + its citations in a single transaction. `listReflections()` returns all reflections. `getReflection(id)` returns a single reflection with citations. `wipeReflections()` drops all rows from `synthesis_reflections` and `synthesis_reflection_citations`. `listStaleReflectionIds()` returns reflection IDs whose cited entries have been modified or deleted since the reflection's `created_at`. `deleteReflections(ids)` removes specific reflections by ID.
