...
- **Conversion output** — Each handler writes into `converted/<rawId>/`:
  ...
  - `meta.json` — `{ rawId, handler, extractedAt, metadata }` where `metadata` is handler-specific (`pageCount`, `wordCount`, `slideCount`, `totalSlideCount`, `hiddenSlideCount`, `slidesToImagesRequested`, `slideImagesWarning`, `rasterizedSlideCount`, `sourceCounts`, `mediaCount`, …).
