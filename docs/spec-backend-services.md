...
**Global settings:** `Settings.knowledgeBase` configures three CLIs separately — **Ingestion** (optional, vision-capable, used for AI-assisted page/slide/image conversion at ingest time), **Digestion**, and **Dreaming** — each with its own `cliBackend`, `cliModel`, `cliEffort`. It also exposes `cliConcurrency` (1–10, default 2) — a per-workspace cap on documents processed in parallel by all three pipelines (within a document, work stays sequential) — and the `convertSlidesToImages` opt-in. Digestion + Dreaming fall back to `Settings.defaultBackend` when unset; Ingestion is opt-in (empty = skip AI conversion, falls back to image-only references). The settings modal exposes cascading backend → model → effort pickers per role plus a numeric input for CLI concurrency.
...
- **Batch mode** — `enqueueBatchDigest(hash)` lists every `ingested` + `pending-delete` raw (`pending-delete` first so purges happen before digestions) and chains each one onto the shared queue. Failures inside the batch don't stop the run — they land on their raw row and the loop continues. Progress is reported through the unified **digestion session counter** (see below) rather than a batch-specific frame.
...
- `convertImageToMarkdown(imagePath, { adapter, model?, effort?, timeoutMs? }) → { markdown, retried }` — a single retry-once wrapper over `adapter.runOneShot` that converts one image (PDF page render, slide render, embedded DOCX figure, or standalone uploaded image) into clean Markdown using the **unified image-to-Markdown prompt** (design §8). Behavior:
  ...
  - Default per-image timeout is 3 minutes (vs. 15 minutes for whole-document digestion).
...
- **`generateTitle(userMessage, fallback)`** — returns a short conversation title. Base class provides a default that truncates the user message to 80 chars. `chatService.generateAndUpdateTitle()` applies an additional 8-word hard-cut on the adapter's return value before persisting, so adapters don't need to enforce a word count themselves.
...
**`generateTitle(userMessage, fallback)`** — spawns `claude --print -p <prompt>` with 30s timeout to generate a short title (prompt asks for max 8 words) from the user's first message. Falls back to truncated user message. Closes stdin immediately via `child.stdin?.end()`. The returned title is further bounded to 8 words by `chatService.generateAndUpdateTitle()`.
