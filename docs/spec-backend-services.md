...
- **Digestion-session counter** — The service holds a per-workspace `sessions` map `{ entriesCreated, pending }`, created lazily when any digest task enqueues (single via `enqueueDigest` or batch via `enqueueBatchDigest`). `_trackPending(hash, ±n)` adjusts the `pending` count on enqueue/settle; `_recordEntriesCreated(hash, n)` increments `entriesCreated` after each raw settles and emits `kb_state_update { digestion: { active: true, entriesCreated } }`. When `pending` drops to zero the service emits exactly one `kb_state_update { digestion: { active: false, entriesCreated } }` frame with the final total and deletes the session entry, so a subsequent enqueue starts a fresh counter at zero. Batch digests bump `pending` by the number of eligible raws up front and decrement per-item inside the loop, so concurrent single digests interleaving with a batch run share the same session and accumulate into a single count-up before the "complete" frame fires. Failed digests still emit the `active: false` frame with whatever `entriesCreated` had been recorded (zero if nothing was created). The KB Browser frontend turns `active: true` frames into a live "N entities created" counter adjacent to the batch progress text, and turns the `active: false` frame into a dismissable "Digestion complete — N entities created" banner that persists until the user closes it or leaves the KB Browser.
