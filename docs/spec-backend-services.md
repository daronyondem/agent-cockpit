...
**Subagents (`collabAgentToolCall`):** Codex emits `collabAgentToolCall` ThreadItems on the parent connection whenever the parent thread invokes a child via the collab protocol. The item shape is `{ id, tool, status, senderThreadId, receiverThreadIds, prompt?, model?, agentsStates }`, where `tool` is one of `spawnAgent | sendInput | resumeAgent | wait | closeAgent`, item-level `status` is `inProgress | completed | failed`, and per-receiver lifecycle (`pendingInit | running | interrupted | completed | errored | shutdown | notFound`) lives in `agentsStates`, keyed by child thread id. Each call is rendered as its own opaque cockpit Agent card (`tool: 'Agent'`, `isAgent: true`, `subagentType: <op>`, `description` derived from the operation kind plus prompt preview). The cockpit cannot render child-thread internals — `item/started` / `item/completed` notifications don't carry `threadId` in their params, so demultiplexing parent vs. child notifications on a single connection isn't supported by the public protocol. Outcome derivation: a receiver `errored` / `notFound` state surfaces as a card error even when the call's item-level `status` is `completed`, so subagent failures don't silently render green.
