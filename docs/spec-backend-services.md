...
**Per-image AI conversion (`src/services/knowledgeBase/ingestion/pageConversion.ts`):** Scaffolding for the hybrid AI-assisted ingestion design (see `docs/design-kb-ingestion-hybrid.md`). Exports `convertImageToMarkdown(imagePath, { adapter, model?, effort?, timeoutMs? }) → { markdown, retried }` — a single retry-once wrapper over `adapter.runOneShot` that converts one image (PDF page render, slide render, embedded DOCX figure, or standalone uploaded image) into clean Markdown using the **unified image-to-Markdown prompt** (design §8). Behavior:
...
- **Downscales oversized inputs** before the AI call: if the image's long edge exceeds **2576 px**, the helper decodes it via `@napi-rs/canvas`, draws a proportionally-scaled copy, encodes to PNG, and writes it to a private `kb-ai-image-*` temp dir; that temp path is what `runOneShot` sees (with `workingDir` and the basename in the prompt rewritten to match). The temp dir is removed in a `finally` block whether the call succeeds or fails. The cap keeps each image under ~4,800 vision tokens for typical aspect ratios and avoids provider-side rejections on multi-MB inputs. Decode failures (e.g. corrupt or unsupported format) fall through to the original path — the CLI's `Read` tool may still handle it. The original on-disk image is **never** modified; handlers reference it from `text.md` at full resolution.
