...
**Mid-turn input (`sendInput`):** The `sendInput(text)` callback returned from `sendMessage()` issues a fire-and-forget `turn/steer({threadId, input: [{ type: 'text', text }], expectedTurnId})` JSON-RPC request, which appends the text to the currently in-flight turn server-side (the model receives it as part of the same turn rather than starting a new one). `expectedTurnId` is required by the protocol; the adapter passes the tracked `turnId` so the server rejects stale UI input rather than silently applying it to a turn the user didn't intend. Guards: no-op when (a) the turn has been aborted (so we don't race the `turn/interrupt` issued by `abort()`), (b) `text` is empty/non-string, or (c) the client/threadId/turnId state hasn't been populated yet (i.e. the first `turn/start` hasn't resolved). Server rejections are logged via `console.warn` and otherwise swallowed so a stale steer never crashes the stream.
