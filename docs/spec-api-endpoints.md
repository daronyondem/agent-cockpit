...
| POST | `/workspaces/:hash/kb/dream` | Yes | Incremental dream run — processes only entries with `needs_synthesis = 1`. **Fire-and-forget:** returns `202 { accepted: true }` immediately. The background job runs the two-phase Discovery→Synthesis pipeline, applies operations transactionally, regenerates `synthesis/` markdown, and emits `kb_state_update` frames with `dreamProgress: { phase, done, total }`. `400` if no entries pending synthesis (`countNeedsSynthesis() === 0`), `400` if KB disabled, `409` if a dream is already running for this workspace. |
