...
`GET /active-streams` merges durable active jobs with runtime `activeStreams`, so accepted/preparing jobs appear before the backend stream object exists. Each `streams[]` item includes `id`, `jobId`, `state`, `backend`, `startedAt`, `lastEventAt`, `connected`, `runtimeAttached`, and `pending`; `ids[]` remains the compatibility projection.
