...
- **Reconnection with state recovery:** On client disconnect, the CLI process is NOT killed. Instead, a 60-second grace period starts. Events continue to be buffered server-side (ring buffer, max 1000 events). If the client reconnects within the grace period, the server replays all buffered events wrapped in `replay_start`/`replay_end` frames, then resumes live streaming. If the grace period expires without reconnection, the CLI is aborted and the server emits synthetic `error` (`"WebSocket reconnect grace period expired"`) + `done` frames into the buffer so a later reconnect (e.g. page refresh after the user came back from a long sleep) sees the closure and clears its in-progress UI state instead of hanging. The buffer is then dropped by the `BUFFER_CLEANUP_MS` (60s) cleanup timer that `send()` arms whenever a `done` arrives without a connected WS. If the CLI completes during the disconnect, events (including `done`) are buffered and replayed on reconnect; the same cleanup timer eventually discards the buffer if no reconnect occurs.
- **POST without an open WebSocket (network-change recovery):** `POST /conversations/:id/message` always spawns the CLI and runs `processStream`, regardless of whether a WebSocket is currently connected for the conversation. If no WS is open at submission time, the chat router calls `wsFns.startStreamGracePeriod(convId)` *before* starting the stream so `isStreamAlive()` stays true while frames buffer. This unblocks the case where a stale browser tab (laptop returning from sleep with a NAT-rebound WebSocket sitting in `readyState === OPEN` but with a dead TCP socket underneath) submits a POST whose user message persists but the CLI never spawns. The grace timer will eventually expire if the client never reconnects, taking the synthetic-error+done path described above.
