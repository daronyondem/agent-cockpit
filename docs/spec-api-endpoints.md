...
| `turn_boundary` | — | Marks boundary between assistant turns (internal — not forwarded to client). Triggers persistence of accumulated `toolActivity` and `contentBlocks` on the intermediate message. The persisted message carries `turn: 'progress'`. |
