...
model, [ADR-0071](adr/0071-support-linux-production-installs.md) extends it to
Linux installs, and [ADR-0063](adr/0063-adopt-per-user-windows-installer.md)
extends it to per-user Windows installs. `scripts/install-macos.sh`,
`scripts/install-linux.sh`, and `scripts/install-windows.ps1` generate the
required runtime secrets and PM2
...
**Mobile PWA auth:** The supported mobile client is the PWA served from `/mobile/`. It uses the same browser-owned `connect.sid` session and CSRF token flow as the desktop V2 UI. When an unauthenticated mobile browser opens `/mobile/`, `requireAuth` redirects to `/auth/login?next=<encoded mobile path>` and successful password, passkey, or recovery-code login returns to that safe relative `/mobile/...` target instead of falling through `/` to the desktop `/v2/` UI. There is no separate native-client pairing bridge, one-time callback, paired-device registry, or device-revocation route in the active auth surface.
...
| `/auth/login` | GET | First-party login page. If no owner exists, redirects to `/auth/setup`. Accepts `?popup=1`; that mode is carried as a hidden field on the password form so successful login can finish popup re-auth. Accepts `?next=<relative path>` only when it is a safe same-origin path beginning with one `/`; unsafe absolute, protocol-relative, backslash, or control-character values are ignored. The safe `next` is carried into password, passkey, and recovery login surfaces. |
| `/auth/login/password` | POST | Password login for the local owner. Form/JSON body `{ email, password, popup?, next? }`. Valid credentials create the normal session and redirect to a safe `next` path when supplied, otherwise `/`; with `popup=1`, it redirects to `/auth/popup-done` and ignores `next`. Invalid credentials return 401 with the login page and an error while preserving safe hidden mode fields. If `policy.passkeyRequired` is true, password login returns 403 and the owner must use passkey login or recovery. |
...
| `/api/auth/passkeys/login/options` | POST | Public + rate limited. Body `{ popup?, next? }`. Requires at least one registered passkey, generates assertion options for all registered credentials, stores challenge and mode in session, and stores only a safe same-origin relative `next` path. |
| `/api/auth/passkeys/login/verify` | POST | Public + rate limited. Body `{ response }`, where `response` is the JSON-encoded `PublicKeyCredential` from `navigator.credentials.get`. Verifies challenge/origin/RP ID/user verification, updates counter/last-used metadata, creates the normal session, and returns `{ redirectTo, user }` so the login page can navigate to the preserved safe `next`, `/`, or `/auth/popup-done`. |
...
| `/auth/recovery/login` | POST | Form/JSON body `{ email, recoveryCode, popup?, next? }`. The code must match an unused stored recovery-code hash. On success, the code is marked used, `passkeyRequired` is disabled, and the session finishes like password login, including popup mode and safe `next` redirects. |
...
Several routes accept user-supplied paths that must resolve inside a workspace
or conversation execution directory. Explorer and Git diff routes accept
workspace-relative paths; file-delivery routes receive absolute paths emitted in
chat messages or generated by the UI. All guarded paths use the same containment
principle:

1. **Normalize route-appropriate path input.** Relative explorer/Git paths strip leading `/` or `\` before `path.resolve(root, rel)` so absolute-looking inputs are treated as workspace-relative. File-delivery paths are resolved as supplied because message markers store absolute paths.
