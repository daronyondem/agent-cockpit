...
- `src/api.ts` — same-origin TypeScript API client for current-user, settings, backends, profile metadata, conversations, streams, queue, sessions, attachments/OCR, file delivery, workspace explorer, and WebSocket URLs. Conversation create, message send, stdin input, and explorer mutation bodies use type-only imports from browser-safe `src/contracts/*` files so mobile request shapes compile against the same contract types as the server. State-changing requests lazily fetch CSRF tokens and send `x-csrf-token`. Multipart uploads use `XMLHttpRequest` for progress and cancel.
