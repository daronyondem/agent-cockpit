...
Claude Code profiles may also carry `claudeCode.provider`, normalized to `"anthropic"` when missing and currently either `"anthropic"` or `"bedrock"` ([ADR-0090](adr/0090-represent-claude-code-bedrock-as-a-cli-profile-provider.md)). Anthropic profiles use the static Claude Code model catalog. Bedrock profiles keep AWS/Claude runtime values in the existing `env` object (`CLAUDE_CODE_USE_BEDROCK`, `AWS_REGION`/`AWS_DEFAULT_REGION`, `AWS_PROFILE`, bearer-token env vars, etc.) and store model routing separately at `claudeCode.bedrock.inferenceProfiles[]`. Each inference profile row is `{ id, name, inferenceProfileId, baseModelId?, default? }`: `name` is the user-facing model label, `inferenceProfileId` is the exact Bedrock inference profile ID or ARN passed to Claude Code as `--model`, `baseModelId` optionally points at a known static Claude model such as `claude-fable-5` so Agent Cockpit can inherit capabilities/effort metadata, and only one row can be default. `SettingsService.saveSettings()` trims row strings, drops all-empty rows, generates stable missing row IDs from the name/profile id, keeps only the first `default:true`, and marks the first row default when none is selected.
...
Usage-cost snapshots keep the actual invoked/reported `model` string. For Claude Code Bedrock inference profile IDs/ARNs, the MVP estimator matches known embedded Claude model ids such as `claude-fable-5` and `claude-opus-4-8` through the Anthropic catalog's wildcard patterns, so estimates continue to use the logical Claude family pricing while preserving the Bedrock model string in the snapshot. A future schema may add an explicit `pricingModelId` if pattern matching becomes too broad.
