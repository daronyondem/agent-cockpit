...
└── data/                               # Runtime data root, default `<repo>/data` and movable with AGENT_COCKPIT_DATA_DIR
    ...
    ├── restart.sh                      # PM2 restart script written by UpdateService when restart/update is requested
...
Production GitHub Releases upload `release-manifest.json` beside the release
tarball and `SHA256SUMS`. The manifest is generated by
`scripts/package-release.js` and is intentionally outside the tarball so the
installer/updater can verify the tarball hash before extraction.
...
`artifacts[]` includes the app tarball and, when present, the external macOS
installer asset uploaded beside the tarball. `files[]` contains every regular
file copied into the tarball, with paths relative to the tarball's package root.
