...
describe('frontend routes', () => {
  ...
  test('context map settings expose global and workspace controls', () => {
    ...
    expect(workspaceSettingsSrc).toContain("scrollIntoView({ behavior: 'smooth', block: 'start' })");
    expect(workspaceSettingsSrc).toContain('Context Map scan started');
    expect(workspaceSettingsSrc).toContain('ws-cm-advanced');
    expect(workspaceSettingsSrc).toContain('Last scan');
    expect(workspaceSettingsSrc.indexOf('Last scan')).toBeLessThan(workspaceSettingsSrc.indexOf('Active Map'));
    ...
    expect(workspaceSettingsSrc).toContain('Save Context Map');
    ...
    expect(cssSrc).not.toContain('.settings-form.ws-form-context-map');
    ...
    expect(cssSrc).toContain('.ws-cm-advanced');
  ...
  test('static frontend surface serves V2 and leaves removed assets unavailable', async () => {
    ...
    await withServer(app, async (server) => {
      ...
      expect(v2.body).toContain('src/workspaceSettings.jsx?v=147');
