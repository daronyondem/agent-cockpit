...
describe('Windows installer script', () => {
  ...
  test('generates runtime config, helper scripts, local PM2 home, and logon startup', () => {
    ...
    expect(source).toContain('run-agent-cockpit.vbs');
    expect(source).toContain("script = $runnerScript");
    expect(source).toContain("interpreter = 'wscript.exe'");
    expect(source).toContain("node_args = @('//B', '//NoLogo')");
    expect(source).toContain('Function Q(value)');
    expect(source).toContain('shell.Run(cmd, 0, True)');
    expect(source).toContain('--import tsx server.ts');
    expect(source).toContain('agent-cockpit-runner-error.log');
    expect(source).toContain('Agent Cockpit did not answer before timeout; collecting PM2 diagnostics.');
    expect(source).toContain("Invoke-Pm2BestEffort $AppDir @('logs', $AppName, '--lines', '80', '--nostream')");
    ...
    expect(source).toContain('function Resolve-Pm2');
    expect(source).toContain("Join-Path `$AppDir 'node_modules\\.bin\\pm2.cmd'");
    expect(source).toContain('Local PM2 command is missing');
    expect(source).toContain('function Invoke-CheckedNative');
    expect(source).toContain('$LASTEXITCODE');
    expect(source).toContain("Invoke-CheckedNative `$Pm2 @('delete', 'agent-cockpit') -AllowFailure");
    expect(source).toContain("Invoke-CheckedNative `$Pm2 @('startOrRestart', (Join-Path `$AppDir 'ecosystem.config.js'), '--update-env')");
    ...
    expect(source).toContain("Invoke-Pm2BestEffort $AppDir @('delete', $AppName)");
    expect(source).toContain('Skipping PM2 cleanup for $AppDir because local PM2 is not installed yet.');
    expect(source).toContain("Invoke-Quiet $pm2Cmd @('startOrRestart', 'ecosystem.config.js', '--update-env') $AppDir");
    expect(source).not.toContain("@('--no-install', 'pm2'");
