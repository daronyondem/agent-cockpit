...
(function(){
  ...
  function refresh(){
    ...
    if (!window.AgentApi || !window.AgentApi.getCliUpdates) {
      return Promise.resolve(cached);
    }
    inFlight = window.AgentApi.getCliUpdates()
      .then(set)
      .catch(err => {
  ...
  function check(){
    if (!window.AgentApi || !window.AgentApi.checkCliUpdates) {
      return Promise.resolve(cached);
    }
    return window.AgentApi.checkCliUpdates().then(set);
  }

  function update(itemId){
    if (!window.AgentApi || !window.AgentApi.triggerCliUpdate) {
      return Promise.reject(new Error('CLI update API is unavailable'));
    }
    return window.AgentApi.triggerCliUpdate(itemId)
      .then(result => {
  ...
  window.CliUpdateStore = { get, refresh, check, update, subscribe, findForSelection };
})();
