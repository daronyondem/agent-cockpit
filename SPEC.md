...
| `test/knowledgeBase.handlers.test.ts` | Per-format KB handlers: **pdfHandler** (real fixture in `test/fixtures/kb/sample.pdf` — per-page H2 sections, filename header, `pageCount`/`wordCount` metadata), **docxHandler** (stubs `runPandoc` via `jest.spyOn` on the pandoc module — happy path with simulated `--extract-media` output verifies flattened media layout, collision-safe rename, GFM table preservation, `media/media/...` reference rewriting, `docx/pandoc` handler tag, and argument shape; no-media branch; and a rejection branch verifying pandoc errors propagate so the orchestrator can mark the entry failed), **pptxHandler** (slide text, speaker notes, embedded media, `slideCount`/`slidesToImagesRequested` metadata, plus a hermetic "LibreOffice missing" test that sets PATH to an empty dir and resets the detection cache to force the fallback warning path), **passthroughHandler** (markdown inlined, non-markdown text wrapped in a code fence, >200 KB truncation marker, image files copied into `media/` with an inline reference, `passthroughSupports` extension matrix), **dispatch** (`pickHandler` extension-first + MIME-fallback, `ingestFile` routes real fixtures through the right handler, `UnsupportedFileTypeError` for unknown extensions). |
