...
**LibreOffice detection** (`src/services/knowledgeBase/libreOffice.ts`): `detectLibreOffice()` runs once at server startup (fire-and-forget, so it never blocks port binding) and caches the result at module level. It uses `which soffice` on POSIX and `where soffice` on Windows — not `soffice --version`, because launching the full binary would spawn a user profile at startup. The cached `LibreOfficeStatus` (`{ available, binaryPath, detectedAt }`) is consulted by the ingestion stage (PR 2) when `Settings.knowledgeBase.convertSlidesToImages` is true; if LibreOffice is unavailable, ingestion logs a warning and falls back to extracting text + speaker notes + embedded media only.
