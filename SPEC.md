...
│       │       ├── pdf.ts              # PDF text extraction via unpdf (per-page H2 sections)
│       │       ├── docx.ts             # DOCX text via mammoth + embedded media via adm-zip
...
| POST | `/workspaces/:hash/kb/raw` | Yes | `multipart/form-data` with a single `file` field (max 50 MB). Hashes the buffer to derive `rawId = sha256[:16]`, stages `raw/<rawId>.<ext>`, and writes the `ingesting` state entry synchronously before returning. A background conversion job is scheduled on the workspace's FIFO queue. Returns **202** with `{ entry: KbRawEntry, deduped: boolean }`. `deduped: true` indicates the identical content already exists — the existing entry is returned unchanged and no new work is scheduled. `400 { error: "KB disabled" }` if KB is off for the workspace, `400` if no file field, `404` if workspace not found, `413` if multer's file-size limit is exceeded. Emits a `kb_state_update` WS frame on every state mutation (stage, in-flight status change, completion). |
...
- **Conversion output** — Each handler writes into `converted/<rawId>/`:
  - `text.md` — extracted text with an `# <filename>` header. PDF sections under `## Page N`, PPTX under `## Slide N` with optional `### Speaker Notes`, DOCX/PPTX with `## Embedded Media` listing referenced media files, passthrough preserving markdown as-is or wrapping other text in a code fence tagged with the file extension.
  - `meta.json` — `{ rawId, handler, extractedAt, metadata }` where `metadata` is handler-specific (`pageCount`, `wordCount`, `slideCount`, `slidesToImagesRequested`, `slideImagesWarning`, `truncated`, `mediaCount`, …).
...
- **`pdf.ts`** — Uses `unpdf` (wraps pdfjs-dist) to extract per-page text. Writes `# <filename>` + per-page `## Page N` sections. Metadata: `pageCount`, `wordCount`. No media extraction.
- **`docx.ts`** — Uses `mammoth.extractRawText` for body text and `adm-zip` to pull entries out of `word/media/` into `converted/<rawId>/media/`. Media filenames are prefixed with the rawId to avoid collisions. Adds an `## Embedded Media` section when media is present.
- **`pptx.ts`** — Uses `adm-zip` + `fast-xml-parser` (`removeNSPrefix: true`) to walk `ppt/slides/slide*.xml`, `ppt/notesSlides/notesSlide*.xml`, and `ppt/media/`. Writes `## Slide N` blocks with extracted text and optional `### Speaker Notes`. When `convertSlidesToImages` is requested at ingestion time, the handler checks the cached `LibreOfficeStatus`; if unavailable, it records `slideImagesWarning` in metadata and injects a "Slide rasterization note" into `text.md` explaining LibreOffice is missing. When available, it shells out to `soffice --headless --convert-to pdf --outdir <tmp>` and then uses `unpdf`'s `renderPageAsImage` (via a dynamically imported `@napi-rs/canvas`) to rasterize each slide into `media/slide-N.png`. Canvas dep is probed via a dynamic import with an `any`-cast wrapper so TypeScript doesn't try to resolve it at compile time — a missing dep produces a warning and a text-only fallback.
...
**Multer configuration:** The `POST /kb/raw` route uses `multer({ storage: multer.memoryStorage(), limits: { fileSize: 50 * 1024 * 1024, files: 1 } }).single('file')`. Memory storage is required because the orchestrator needs to hash the buffer before touching disk (so dedup works and we never write a file the user already has).
...
**Framework:** Jest 30.x — run with `npm test`. The script is `node --experimental-vm-modules node_modules/.bin/jest` because the KB handler tests dynamically import `unpdf` (ESM-only, wraps `pdfjs-dist`) and Jest's VM needs the flag to allow runtime `import()` calls.
